...
class State:
    ...
    def __str__(self):
        return f"{self.next_rock.to_value()},{self.next_wind_turbine.to_value()},{self.distance_next_wind_turbine},{self.bloc_type}"
    ...
    def from_str(state_str):
        next_rock, next_wind_turbine, tower_distance, block_type = state_str.split(",")
        return State(Radar.from_value(int(next_rock)), Radar.from_value(int(next_wind_turbine)), tower_distance, block_type)
